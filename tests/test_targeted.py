...
def make_feature(id: str = "f1", mag=1.0, coords=None, extra_props=None):
    ...
    return {"type": "Feature", "id": id, "properties": props, "geometry": {"type": "Point", "coordinates": coords}}
...
def test_cache_upsert_updates_row():
    with tempfile.TemporaryDirectory() as td:
        ...
        insert_raw_event(conn, evt1.id, json.dumps(f1), time=evt1.time, updated=evt1.updated, mag=evt1.mag, depth=evt1.depth, lat=evt1.lat, lon=evt1.lon, place=evt1.place, urls=json.dumps(evt1.urls) if evt1.urls else None, tsunami=evt1.tsunami)

        # upsert same id with different mag
        f2 = make_feature(id="dup1", mag=4.5)
        evt2 = normalize_feature(f2)
        insert_raw_event(conn, evt2.id, json.dumps(f2), time=evt2.time, updated=evt2.updated, mag=evt2.mag, depth=evt2.depth, lat=evt2.lat, lon=evt2.lon, place=evt2.place, urls=json.dumps(evt2.urls) if evt2.urls else None, tsunami=evt2.tsunami)
...
def test_score_edgecases_missing_time_and_extremes():
    ...
    # recency_hours should use sentinel when time missing (9999.0)
    assert res["factors"]["recency_hours"] == 9999.0
